...
import { FaCashRegister, FaCircleCheck } from "react-icons/fa6";
...
export default function PopupVenda(props: PopupVendaProps) {
  ...
  useEffect(() => {
    if (props.produto && selectedFichaData) {
      const maxCompraFicha = Math.trunc(
        selectedFichaData.saldo / props.produto.preco
      );
      if (maxCompraFicha == 0) {
        setFicha(0);
        setQuantidade(0);
      } else {
        setQuantidadeMax(Math.min(maxCompraFicha, props.produto.estoque));
      }
    }
  }, [props.produto, selectedFichaData]);
        ...
        <>
          ...
          {props.produto.estoque > 0 ? (
            <form onSubmit={!fetchingCreate ? handleCreateVenda : undefined}>
              <div className="line responsive">
                <Input
                  ...
                  options={fichas?.map((ficha) => ({
                    ...
                    disabled:
                      props.produto && ficha.saldo < props.produto.preco,
                  }))}
                  disabledValue={0}
                  value={ficha}
              ...
              <div className="line">
                <Tag
                  className="medium flex-1 center"
