...
export type Produto = NovoProduto & {
  ...
  limite_reserva?: number;
  quantidade_reserva_disponivel?: number;
  total_reservas_antecipadas?: number;
};

export const createMap = (produtos: Produto[]) =>
  produtos.reduce(
...
// POST
export const postProduto = async (nP: NovoProduto) =>
  await api.post("movimentacao/produtos/", nP);

// GET
export const getProdutos = async (search?: string) =>
  await api.get(`movimentacao/produtos/${search ? `?search=${search}` : ""}`);
...
export const patchProduto = async (
  id: number,
  dadosParciais: Partial<NovoProduto>
) => {
...
export const deleteProduto = async (id: number) =>
  await api.delete(`movimentacao/produtos/${id}/`);
